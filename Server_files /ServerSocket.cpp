...
class ServerSocket {
    ...
    static int bind_socket_to_addr(addrinfo *result_of_getaddr) {
        ...
        for (p = result_of_getaddr; p != nullptr; p = p->ai_next) {
            sock_fd = sock_fd(p->ai_family, p->ai_socktype, p->ai_protocol);
    ...
    void start_listen() {
        ...
        cout << "Server is listening port " << PORT << endl;
    }
...
public:
    ServerSocket(const char* IP, const char* PORT, const int backlog) : IP(IP), PORT(PORT), backlog(backlog) {}
